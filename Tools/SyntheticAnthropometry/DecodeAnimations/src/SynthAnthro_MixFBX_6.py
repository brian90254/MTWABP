...
def extract_bind_pose_matrices(text):
    """Extracts bind pose matrices per bone name from PoseNode blocks."""
...
def replace_matrix_values(text, matrices1, matrices2):
    """Averages and replaces bind pose matrices in PoseNode blocks."""
...
def replace_block(text, span, new_array, values_per_line=10, add_prefix=True):
    ...
    if span:
        ...
        return text  # skip if span is None
...
def average_ascii_fbx_with_bones(text1, text2):
    ...
    # Bind Poses
    bind1 = extract_bind_pose_matrices(text1)
    bind2 = extract_bind_pose_matrices(text2)
    text1 = replace_matrix_values(text1, bind1, bind2)
...
def main(fbx1_path, fbx2_path, output_path):
    ...
    print(f"✅ Blended FBX with bind poses saved to: {output_path}")
