...
def extract_weights(text):
    matches = re.findall(r'Indexes: \*[^}]+?\{[^}]+?a: ([^\}]+)\}', text)
    weights = re.findall(r'Weights: \*[^}]+?\{[^}]+?a: ([^\}]+)\}', text)
    return matches, weights

# indexes_f, weights_f = extract_weights(text_f)
# indexes_m, weights_m = extract_weights(text_m)
# print(indexes_f == indexes_m)
# print(weights_f == weights_m)
...
def extract_cluster_blocks(text):
    return dict(re.findall(r'(Deformer: "SubDeformer::([^\"]+)", "Cluster"\s*\{[\s\S]+?\})', text))

def average_cluster_blocks(text_f, text_m):
    clusters_f = extract_cluster_blocks(text_f)
    clusters_m = extract_cluster_blocks(text_m)
    output_text = text_f

    for name, block_f in clusters_f.items():
        block_m = clusters_m.get(name)
        if not block_m: continue
        ...
        if tf_f is None or tf_m is None or tfl_f is None or tfl_m is None:
            continue
        ...
        new_block = re.sub(r'(Transform: \*16 \{\s*a:\s*)([^\}]+)(\s*\})', lambda m: f'{m.group(1)}{tf_str}{m.group(3)}', block_f)
        new_block = re.sub(r'(TransformLink: \*16 \{\s*a:\s*)([^\}]+)(\s*\})', lambda m: f'{m.group(1)}{tfl_str}{m.group(3)}', new_block)
        output_text = output_text.replace(block_f, new_block)
...
def main(female_file, male_file, output_file):
    ...
    output_text = average_cluster_blocks(output_text, text_m)
