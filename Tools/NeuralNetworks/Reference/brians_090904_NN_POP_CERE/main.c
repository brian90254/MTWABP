...
float VisWij [MAX_NUM_NEURONS] [MAX_NUM_NEURONS]; // weight matrix


int printWeightFlag = 0;
int homeostasisFlag = 0;
int RandomInputFlag = 0;
int BlankInputFlag = 0;
...

{
	...
	*/
	...
	if (argc != 5)
	{
		fprintf (stderr, "USAGE: ./nn_alpha_1 connectivity.csv weights.csv threshold.csv learningRate\n");
		exit (1);
	}
	...
	else
	{
		...
		{
			fprintf (stderr, "ERROR: learning rate must be greater than zero\n");
			exit (1);
		}
		else
		{
