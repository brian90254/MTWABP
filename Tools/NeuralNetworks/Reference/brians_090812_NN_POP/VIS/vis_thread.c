...
// --------------------------
	...
	IplImage *imgWeight1 = cvCreateImage( cvSize( 13 * SCALE_FACTOR, 13 * SCALE_FACTOR ), 8, 3 );
	cvNamedWindow( "WeightOne", 1 );	
	cvShowImage( "WeightOne", imgWeight1 );	
	cvMoveWindow( "WeightOne", xPos + (16 + 14) * S_F, yPos + 9 * S_F);
	
	IplImage *imgWeight2 = cvCreateImage( cvSize( 13 * SCALE_FACTOR, 1 * SCALE_FACTOR ), 8, 3 );
	cvNamedWindow( "WeightTwo", 1 );	
	cvShowImage( "WeightTwo", imgWeight2 );	
	cvMoveWindow( "WeightTwo", xPos + (16 + 14) * S_F, yPos + 9 * S_F + 15 * S_F);
	...
	srand(time(0)); // seed variable
	 ...
	 while (quit == NO_EXIT) 
	 {
		...
		//if(RandCountDown > 25)
		{
			...
			// TEST TWO
			fTemp_vis *= (.6); // normalize 0-1
			fTemp_vis += .3; // normalize 0-1
		 ...
		 for (vi = 0; vi < 52; vi++ )
		 {
			 ...
			 neuronOutLoc = cvPoint((xTemp * S_F) + 13, (yTemp * S_F) + 13);
			 neuronOutRadius = 10;
			 drawNeuronCircle(imgGauss2, Vis_Ui[vi + 54], neuronOutLoc, neuronOutRadius);
			 //drawNeuronCircle(imgGauss1, 0.5, neuronOutLoc, neuronOutRadius); //TESTING
		 }
		 ...
		 for (vi = 0; vi < (169); vi++ )
		 {
			 ...
			 drawNeuronCircle(imgPop1, Vis_Ui[vi + 106], neuronOutLoc, neuronOutRadius);
			 //drawNeuronCircle(imgGauss1, 0.5, neuronOutLoc, neuronOutRadius); //TESTING
		 }
	
		 // ==================================
		 // ----------------------------------
		 // Weight One Visualization
		 // ----------------------------------
		 // ==================================
		 
		 weightMax = 0.25; // nominal zero
		 for (vj = 106; vj < (275); vj++ )
		 {
			 vi = 275;
		 ...
		 for (vj = 0; vj < (169); vj++ )
		 {
			 ...
			 neuronOutLoc = cvPoint((xTemp * S_F) + 13, (yTemp * S_F) + 13);
			 neuronOutRadius = 10;
			 drawNeuronCircle(imgWeight1, (VisWij[vi][vj + 106] / weightMax), neuronOutLoc, neuronOutRadius);
		 }

		 // ==================================
		 // ----------------------------------
		 // Weight Two Visualization
		 // ----------------------------------
		 // ==================================
		 
		 weightMax = 0.25; // nominal zero
		 for (vj = 315; vj < (327); vj++ )
		 {
			 vi = 275;
			 if(VisWij[vi][vj] > weightMax)
			 {
		 ...
		 for (vj = 0; vj < (13); vj++ )
		 {
			 ...
			 neuronOutLoc = cvPoint((xTemp * S_F) + 13, (yTemp * S_F) + 13);
			 neuronOutRadius = 10;
			 drawNeuronCircle(imgWeight2, (VisWij[vi][vj + 315] / weightMax), neuronOutLoc, neuronOutRadius);
		 }
		 ...
		 for (vi = 0; vi < 52; vi++ )
		 {
			 ...
			 neuronOutLoc = cvPoint((xTemp * S_F) + 13, (yTemp * S_F) + 13);
			 neuronOutRadius = 10;
			 drawNeuronCircle(imgGauss3, Vis_Ui[vi + 276], neuronOutLoc, neuronOutRadius);
			 //drawNeuronCircle(imgGauss1, 0.5, neuronOutLoc, neuronOutRadius); //TESTING
		 }
		 ...
		 switch (c)
		 {
			 ...
			 case 'p':
			 case 'P':
				 CereLearningFlag = !CereLearningFlag; // toggle flag
				 printf("cerebellar learning flag = %i\n", CereLearningFlag);
				 break;
			 ...
			 case 'u':
				 ...
				 errorOutput = (avgOutput - Neur_Ui[275]) * 100;
				 printf("InputOne=%f, InputTwo=%f, Avg=%f, NeurOut=%f, Error=%f\n", InputUnitOne, InputUnitTwo, avgOutput, Neur_Ui[275], errorOutput);
