...
def main():
    ...
    try:
        ...
        if str(rush_yds).lower() != "bye" and str(points).lower() != "bye":
            #InputUnitOne = (float(rush_yds) / 400.0) + 0.2
            #InputUnitTwo = (float(points) / 65.0) + 0.2
...
#             # Load next week's input after saving current
#             current_week_index += 1
#             if current_week_index < len(all_weeks):
#                 next_week = all_weeks[current_week_index]
#                 rush_yds = df_stats.at["Rush Yds", next_week]
#                 points = df_stats.at["Points", next_week]
#                 if str(rush_yds).lower() != "bye" and str(points).lower() != "bye":
    ...
    while not quit_flag:
        ...
        if loops_since_input_change == 100 and df_stats is not None and current_week_index < len(all_weeks):
            ...
            try:
                ...
                out_path = f"Outputs/PopOne_{team}_{role}_{week_col}.csv"
                df_pop.to_csv(out_path, index=False)
                print(f"\n[Saved] Loop {loop_count} → {out_path}")
            except Exception as e:
                print(f"Error saving PopOne activations: {e}")

            # Now move to next week
            current_week_index += 1
            loops_since_input_change = 0
