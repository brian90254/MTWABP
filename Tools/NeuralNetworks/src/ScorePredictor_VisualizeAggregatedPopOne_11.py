...
for idx, indicator in enumerate(indicators):
    ...
    if keyA in popone_matrices and keyB in popone_matrices:
        ...
        if cx is not None:
            ...
            #predictedScore = ((cx/20) - 0.05) * 90
            predictedScore = round(((cx / 20) - 0.05) * 90, 1)
            print(f"Centroid {home_team}_Def x {away_team}_Off → X: {cx:.2f}, Y: {cy:.2f}, Predicted Score {predictedScore:.1f}")
            #output_data.append((indicator, matchup_A, cx, cy))
            output_data.append((indicator, matchup_A, predictedScore))
    ...
    if keyC in popone_matrices and keyD in popone_matrices:
        ...
        if cx is not None:
            ...
            #output_data.append((indicator, matchup_B, cx, cy))
            #predictedScore = ((cx/20) - 0.05) * 90
            predictedScore = round(((cx / 20) - 0.05) * 90, 1)
            print(f"Centroid {home_team}_Off x {away_team}_Def → X: {cx:.2f}, Y: {cy:.2f}, Predicted Score {predictedScore:.1f}")
            #output_data.append((indicator, matchup_B, cx, cy))
            output_data.append((indicator, matchup_B, predictedScore))
...
# === SAVE RESULTS TO CSV ===
#df_results = pd.DataFrame(output_data, columns=["INDICATOR", "MATCHUP", "X", "Y"])
df_results = pd.DataFrame(output_data, columns=["INDICATOR", "MATCHUP", "SCORE"])
os.makedirs("Outputs", exist_ok=True)
output_csv_path = f"Outputs/Centroid_Results_{home_team}_vs_{away_team}.csv"
df_results.to_csv(output_csv_path, index=False)
print(f"Saved centroid results to {output_csv_path}")
print(df_results)