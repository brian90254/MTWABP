...
# HOW TO USE:
# python src/ParametricPattern_KnitGlove_18.py CONFIG/ConversionRatios_2.txt
...
# lengthPalm         = float(ratios.get("lengthPalm", 11.5))
# circumferencePalm  = float(ratios.get("circumferencePalm", 23))
# patternType        = str(ratios.get("patternType", "B")).strip().upper()
# handedness         = str(ratios.get("handedness", "R")).strip().upper()

# palmWidthRatio     = float(ratios.get("palmWidthRatio", 1.696))
# palmHeightRatio    = float(ratios.get("palmHeightRatio", 3.50))
# wristHeightRatio   = float(ratios.get("wristHeightRatio", 1.391))
# thumbDropRatio     = float(ratios.get("thumbDropRatio", 0.45))
# thumbHeightRatio   = float(ratios.get("thumbHeightRatio", 5))
# fingerHeightRatio  = float(ratios.get("fingerHeightRatio", 4))
# castoffRow         = int(ratios.get("castoffRow", 1))
# buffer             = int(ratios.get("buffer", 1))
...
# (thumb drop drawing continues from your existing implementation)
...
# === CABLE KNIT ===
# (copy your existing cable drawing logic here)
# ----------------------
# === PALM CABLE KNIT ===
# ----------------------
...
# # BRIAN COX copyright 2025
# # HOW TO USE:
# # python src/ParametricPattern_KnitGlove_15.py <lengthPalm> <circumferencePalm> <patternType> <handedness>
# # Example: python src/ParametricPattern_KnitGlove_15.py 11.5 23 B R
# # This script generates a parametric knit glove pattern BMP with ribbing and cables.
...
# # # LONG PALM
# # palmHeightRatio = 3.50 # 56 courses total
# # # ----------------------------
# # # WRIST
...
# parser.add_argument("handedness", choices=["R", "L"], help="'R' for Right, 'L' for Left")
# # File name for configs
# parser.add_argument("ratiosFile", help="Path to the ConversionRatios.txt file")
# args = parser.parse_args()
...
# # CONVERSION RATIOS
# # ----------------------------
# # Load conversion ratios from file
# ratios = load_conversion_ratios(args.ratiosFile)
# # Assign values from file
# palmWidthRatio    = ratios.get("palmWidthRatio", 1.696)
# palmHeightRatio   = ratios.get("palmHeightRatio", 3.50)
# wristHeightRatio  = ratios.get("wristHeightRatio", 1.391)
# thumbDropRatio    = ratios.get("thumbDropRatio", 0.45)
# thumbHeightRatio  = ratios.get("thumbHeightRatio", 5)
# fingerHeightRatio = ratios.get("fingerHeightRatio", 4)
# castoffRow        = int(ratios.get("castoffRow", 1))
# buffer            = int(ratios.get("buffer", 1))
...
# thumb_drop_height = 2
# # BRIAN'S CODE HERE
# # thumbHeight = thumbWidth // thumbHeightRatio
# # num_thumb_drops = (thumbWidth // 2) + thumbHeight
# thumbHeight = int(thumbWidth // thumbHeightRatio)
# num_thumb_drops = int((thumbWidth // 2) + thumbHeight)
...
# # ----------------------
# # === MIRROR PALM SECTION (LEFT TO RIGHT) ===
# # Only apply if handedness is Left
# # ----------------------
# if args.handedness == 'L':
#     left_inner_x = start_x + 3
#     right_inner_x = end_x - 4
#     bottom_y = start_y - 1  # top of WRIST RIB
#     top_y = finger_end_y + 1  # bottom of FINGER RIB

#     print(f"bottom_y = {bottom_y}")
#     print(f"top_y = {top_y}")
#     print(f"height y = {bottom_y - top_y}")

#     region = image[top_y:bottom_y + 1, left_inner_x:right_inner_x + 1]
#     mirrored = cv2.flip(region, 1)

#     if mirrored.size == 0:
#         print("❌ Mirrored region is empty — check your slice bounds.")
#         print(f"  top_y = {top_y}, bottom_y = {bottom_y}")
#         print(f"  left_inner_x = {left_inner_x}, right_inner_x = {right_inner_x}")
#     else:
#         print(f"mirrored.size = {mirrored.size}")
#         image[top_y:bottom_y + 1, left_inner_x:right_inner_x + 1] = mirrored
...
# os.makedirs("images", exist_ok=True)
# timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
# filename = f"ParametricPattern_KnitGlove_{timestamp}_Cable{args.patternType}_{args.handedness}.bmp"
# filepath = os.path.join("images", filename)
# cv2.imwrite(filepath, image)
