...
from PySide6.QtWidgets import QListView, QTreeView, QHeaderView  # NEW
...
def _enable_sorting_file_dialog(dlg: QFileDialog):
    """
    Enable alphabetical sorting on the Qt (non-native) file dialog views.
    Works for both list and details view.
    """
    # List view (icon/list mode)
    lv = dlg.findChild(QListView, "listView")
    if lv is not None:
        lv.setSortingEnabled(True)
        if lv.model():
            lv.model().sort(0, Qt.AscendingOrder)

    # Tree view (details mode)
...
def open_files_sorted(parent, caption, directory, filter_str) -> list[str]:
    """
    Open a multi-select file dialog that shows files sorted A→Z.
    Returns a list of absolute paths (or [] if cancelled).
    """
    dlg = QFileDialog(parent, caption, directory, filter_str)
    dlg.setOption(QFileDialog.DontUseNativeDialog, True)   # force Qt dialog so we can sort
    dlg.setFileMode(QFileDialog.ExistingFiles)
    dlg.setViewMode(QFileDialog.Detail)                    # details view (has header sorting)
    # Ensure the internal views exist & get sorted after the dialog shows:
    QTimer.singleShot(0, lambda: _enable_sorting_file_dialog(dlg))
    return dlg.selectedFiles() if dlg.exec() else []

def open_file_sorted(parent, caption, directory, filter_str) -> str:
    """
    Single-file version of open_files_sorted. Returns '' if cancelled.
    """
    files = open_files_sorted(parent, caption, directory, filter_str)
    return files[0] if files else ""

def save_file_sorted(parent, caption, directory, filter_str, default_name="") -> str:
    """
    Save dialog (sorting not really needed, but we keep the same look).
...
class LayerPanel(QWidget):
    ...
    def on_add(self):
        base = str(Path("DXF").resolve())
        paths = open_files_sorted(self, "Add DXF Files", base, "DXF Files (*.dxf)")
        if not paths:
            return
        for i, p in enumerate(paths):
            # (keep your color logic or use native colors by calling load_layer(p) without a color)
            color = PALETTE[(len(self.renderer.layers) + i) % len(PALETTE)]
            try:
                self.renderer.layers.append(load_layer(p, color))
            except Exception as ex:
    ...
    def on_import_csv(self):
        base = str(Path.cwd())
        path = open_file_sorted(self, "Import Visible Layers CSV", base, "CSV Files (*.csv)")
        if not path:
            return
