...
// *******************************
// The 40 matrix switches are exposed to MPF in reverse physical order:
//   matrix-0-0 reads physical matrix switch 39
//   matrix-0-1 reads physical matrix switch 38
//   ...
//   matrix-1-7 reads physical matrix switch 0
// Only the OPP reporting order is reversed. Matrix scanning and the mirrored
// FAST outputs continue to use their original physical order.
//
...
unsigned long oppGetInputState(unsigned char address)
{
  ...
  {
    // Card 0 contains MPF logical switches 0 through 31.
    for (unsigned char switchNumber = 0; switchNumber < 32; switchNumber++)
    {
      unsigned char physicalSwitch = (OPP_SWITCH_COUNT - 1) - switchNumber;
      if (switchStateNow[physicalSwitch] == SWITCH_ACTIVE_LEVEL)
      {
  ...
  {
    // Card 1 contains MPF logical switches 32 through 39.
    for (unsigned char switchNumber = 0; switchNumber < 8; switchNumber++)
    {
      unsigned char logicalSwitch = switchNumber + 32;
      unsigned char physicalSwitch = (OPP_SWITCH_COUNT - 1) - logicalSwitch;
      if (switchStateNow[physicalSwitch] == SWITCH_ACTIVE_LEVEL)
      {
        state &= ~(1UL << switchNumber);
      }
