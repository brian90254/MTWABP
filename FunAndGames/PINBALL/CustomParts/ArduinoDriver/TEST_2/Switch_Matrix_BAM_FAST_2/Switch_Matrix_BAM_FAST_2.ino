...
//
// OPP inputs are active-low. That matches this matrix code: a raw state of 0
// is an active switch and a raw state of 1 is an inactive switch.
...
unsigned long oppGetInputState(unsigned char address)
{
  ...
  if (address == OPP_CARD_0)
  {
    ...
    {
      if (switchStateNow[switchNumber] == 0)
      {
  ...
  else if (address == OPP_CARD_1)
  {
    ...
    {
      if (switchStateNow[switchNumber + 32] == 0)
      {
        state &= ~(1UL << switchNumber);
      }
    }
